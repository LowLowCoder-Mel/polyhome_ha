...
#mo: recvId: BAE3040042ae6a2f1c92  通道: BAE3000093d3959e3014
CMD_SEARCH_HOST = {
    "arg": {
        "version": "1.0.3"
    },
...
CMD_GET_HOST_ROOM_LIST = {
    "arg": {
        "hostId": "BAE3040042ae6a2f1c92"
    },
    "cmd": "GetHostRoomList",
    "direction": "request",
    "recvId": "BAE3040042ae6a2f1c92",
    "sendId": "BAC1EC00112233445566"
}
...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    def event_tcp_backaudio_recv_handle(call):
        ...
        if json_data['cmd'] == 'NotifyDevStat':
            # {'sendId': 'BA500000BDIDOH3JP3QJ', 'arg': {'devStat': 'close'}, 'cmd': 'NotifyDevStat', 'recvId': 'FFFFFFFFFF', 'direction': 'request'}
        ...
        if json_data['cmd'] == 'GetPlayStat' or json_data['cmd'] == 'NotifyPlayStat' and json_data['direction'] == 'request':
            for player in media_players:
                if player._roomId == json_data['sendId']:
...
class BackAudio(MediaPlayerDevice):
    ...
    def get_all_channel_id(self):
        CMD_GET_HOST_ROOM_LIST['arg']['hostId'] = self.device_id
        # print('获取所有房间id')
        data = self._send_cmd(CMD_GET_HOST_ROOM_LIST)
        data_l = str(data[4:], 'utf-8')
        data_r = json.loads(data_l) 
        if data_r['sendId'] == self.device_id:
            roomList = data_r['arg']['roomList']
            # print('roomList' + str(roomList))
            for value in roomList:
                self._roomId = value['roomId']
                self._roomName = value['roomName']
            self.get_device_state()

    def set_volume_muted(self, muted):
        CMD_MUTE_MUTE['recvId'] = self._roomId
        if muted:
    ...
    def turn_on(self):
        """turn on"""
        self.get_all_channel_id()
        CMD_OPEN['recvId'] = self._roomId
        # print('turn on' + self._roomId)
    ...
    def turn_off(self):
        """turn off"""
        CMD_CLOSE['recvId'] = self._roomId
        # print('turn off' + self._roomId)
    ...
    def set_volume_level(self, volume):
        ...
        CMD_SET_VOLUME['recvId'] = self._roomId
    ...
    def media_pause(self):
        """Send pause command."""
        CMD_PLAY_PAUSE['recvId'] = self._roomId
    ...
    def media_previous_track(self):
        """previous song"""
        CMD_PLAY_PREV['recvId'] = self._roomId
    ...
    def media_next_track(self):
        """next song"""
        CMD_PLAY_NEXT['recvId'] = self._roomId
        ...
        for key, value in SOURCES.items():
            if source == value:
                ...
                if key == 2: # 本地
                    ...
                    CMD_CHANGE_SOURCE['recvId'] = self._roomId
