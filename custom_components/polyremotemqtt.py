...
DOMAIN = 'polyremotemqtt'

DATA_MQTT = 'poly_remote_mqtt'
...
async def async_setup(hass: HomeAssistantType, config: ConfigType) -> bool:
    ...
    def mqtt_call_services_handler(topic, payload, qos):
        try:
            ...
            hass.add_job(hass.services.call, domain, service, data)
    ...
    def mqtt_pub_state_change_service(call):
        print('Remote Mqtt State Change')
        msg = call.data.get('data')
    ...
    def host_unbind():
        ...
        data_str = json.dumps(data_obj)
        mqtt.async_publish(MQTT_TOPIC_UNBIND_CLIENT + uuid_str, data_str, 0, False)
